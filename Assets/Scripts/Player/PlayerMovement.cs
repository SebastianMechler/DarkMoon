...
public class PlayerMovement : MonoBehaviour
{
  ...
  private MovementState m_crouchState;
...
void UpdatePlayerState()
  ...
  {
    float height = Mathf.Clamp(m_capsuleCollider.height += value * 1.0f * Time.fixedDeltaTime, m_crouchHeightMin, m_crouchHeightMax);
    m_capsuleCollider.height = height;//= Mathf.Clamp(m_capsuleCollider.height += value * 1.0f * Time.fixedDeltaTime, m_crouchHeightMin, m_crouchHeightMax);

    //Vector3 newVector, playerPos;
    //newVector = m_capsuleCollider.center;
    //newVector.y = (value < 0.0f ? -0.5f : 0.0f);
    //m_capsuleCollider.center = newVector;

    //Vector3 cam = Camera.main.transform.position;
    //cam.y += (value < 0.0f ? -m_offset : m_offset);
    //cam.y = Mathf.Clamp(cam.y, 0.3f, 1.5f);
  ...
  public bool CanUncrouch()
  {
    ...
    Vector3 caps = new Vector3(this.gameObject.transform.position.x, this.gameObject.transform.position.y + m_capsuleCollider.center.y, this.gameObject.transform.position.z);

    Ray rayMiddle = new Ray(caps, Vector3.up);
    Ray rayLeft = new Ray(caps + Vector3.left * rayScale, Vector3.up);
    Ray rayRight = new Ray(caps + Vector3.right * rayScale, Vector3.up);
    Ray rayForward = new Ray(caps + Vector3.forward * rayScale, Vector3.up);
    Ray rayBack = new Ray(caps + Vector3.back * rayScale, Vector3.up);
